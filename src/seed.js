export default async function seedDatabase(firebaseDb, collection, addDoc) {
	try {
		await addDoc(collection(firebaseDb, 'projects'), {
			...
			conceptsApplied:
				'Fetch data from api and use the data to update the UI,props,useState and useEffect hook,conditional rendering,grid ,etc.',
		})
		await addDoc(collection(firebaseDb, 'projects'), {
			...
			techUsed: 'HTML,CSS,React Js',
			conceptsApplied:
				'Conditional rendering,props,useState and useEffect hook ,etc.',
		})
		await addDoc(collection(firebaseDb, 'projects'), {
			...
			conceptsApplied:
				'Fetch data from api and use the data to update the UI using DOM manipulation',
		})
		await addDoc(collection(firebaseDb, 'projects'), {
			img: [
				'p_4_1.png',
			...
			overview:
				'A full responsive design website built using mobile first design approach having 3 pages home,article and about me page.',
			techUsed: 'HTML,CSS',
			conceptsApplied:
				'Relative units like ems and rems,media queries,flexbox,grid ,etc.',
		})
		await addDoc(collection(firebaseDb, 'projects'), {
			...
			overview:
				'Tindog a dating app for dogs where a user can like or dislike a profile which triggers a like or nope badge according to user choice of click.',
			techUsed: 'HTML,CSS,Javascript',
			conceptsApplied:
				'Javascript classes to create different dogs object,DOM manipulation ,etc.',
		})
		await addDoc(collection(firebaseDb, 'projects'), {
			...
			techUsed: 'HTML,CSS,Javascript',
			conceptsApplied:
				'Conditional rendering,useState hook, basic javascript logic',
		})
		await addDoc(collection(firebaseDb, 'projects'), {
			...
			techUsed: 'HTML,CSS,Javascript',
			conceptsApplied:
				'Interacting with api using fetch and updating color schemes using DOM manipulation ,etc.',
		})
	} catch (err) {
